...
extension TupleExprElementListSyntax {
    ...
    func extractUserDefaultsParam(canReturnShortenVersion: Bool = false) -> String? {
        guard
            let tupleExprElementSyntax = tupleExprElementSyntaxLabeled(Self.userDefaultsParamLabel),
    ...
    func extractKeyParam() -> String? {
        guard let tupleExprElementSyntax = tupleExprElementSyntaxLabeled(Self.keyParamLabel) else {
            return nil
        }
    ...
    func extractDefaultValueParam() -> String? {
        guard let tupleExprElementSyntax = tupleExprElementSyntaxLabeled(Self.defaultValueLabel) else {
            return nil
        }
    ...
    func extractAccessLevelParam() -> AccessLevel? {
        guard
            let tupleExprElementSyntax = tupleExprElementSyntaxLabeled(Self.accessLevelLabel),
            let memberAccessExprSyntax = tupleExprElementSyntax.expression.as(MemberAccessExprSyntax.self)
        else {
            return nil
        }

        let accessLevelString = memberAccessExprSyntax.base == nil ?
