...
class BaseTestCase: XCTestCase {
    ...
    func expandedPropertySource(for variable: Variable) -> String {
        ...
        if let userDefault = variable.attribute?[.userDefaults] {
            ...
            if
                parts.count == 1,
                let shortNameSubstring = parts.first
            {
                userDefaultsInstanceString = UserDefaults.Name.shorten(String(shortNameSubstring)).asFullName.description
            } else {
        ...
        if let defaultValue = defaultValue {
            registerDefaultValue = "\(userDefaultsInstanceString).register(defaults: [\(storageKey): \(defaultValue)])\n\(Trivia.spaces(8))return "
        } else {
