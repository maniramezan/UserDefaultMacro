import SwiftSyntax
    ...
    struct Variable: CustomStringConvertible {
        ...
        var description: String { declaration() }

        var declarationKeyword: String {
            if case .immutable = declaration { return "let" }
            return "var"
        }

        var isMutable: Bool {
            if case .mutable = declaration { return true }
            return false
        }

        func declaration(currentIndent: Trivia = .spaces(0), extraAttribute: SwiftAttribute? = nil) -> String {
            let attributes = Set([attribute, extraAttribute].compactMap { $0 })
            let attributeString = attributes.map { attribute in "\(attribute.description)\n\(currentIndent.description)"
            }.joined()
        ...
        private var body: String {
            guard case .computational(let body) = declaration else { return "" }
        ...
        private var initializer: String {
            guard let initValue = initValue else { return "" }
