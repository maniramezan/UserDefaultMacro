...
final class UserDefaultPropertyMacroTests: BaseTestCase {
    ...
    let testMacros: [String: Macro.Type] = [
        "UserDefaultProperty": UserDefaultPropertyMacro.self,
    ]
    ...
    func testCustomKeyAndCustomShortUserDefaults() {
        let variable = createAttributedVariable(userDefaults: Self.customUserDefaultsName, key: customStringLiteralKey)
        assertMacroExpansion(
            variable.description,
            expandedSource: expandedPropertySource(for: variable),
            macros: testMacros
